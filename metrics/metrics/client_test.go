...
func TestCollectorClient(t *testing.T) {
	// 启动服务器
	server := NewCollectorServer("localhost:8082")
	...
	// 创建客户端
	client := NewCollectorClient("http://localhost:8082")
	...
	// 测试发送数据
	if err := client.CollectMetricsEvent(testData); err != nil {
...
func TestCollectorClientConcurrency(t *testing.T) {
	// 启动服务器
	server := NewCollectorServer("localhost:8083")
	errChan := make(chan error, 1)
	go func() {
		if err := server.Start(); err != nil && err != http.ErrServerClosed {
			errChan <- err
	...
	// 创建客户端
	client := NewCollectorClient("http://localhost:8083")
	...
	for i := 0; i < numGoroutines; i++ {
		go func(index int) {
			...
			if err := client.CollectMetricsEvent(testData); err != nil {
				t.Errorf("Failed to collect metrics in goroutine %d: %v", index, err)
			}
			done <- true
		}(i)
	}
