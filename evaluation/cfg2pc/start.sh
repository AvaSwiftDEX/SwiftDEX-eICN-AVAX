...
echo ">>> >>> >>> start deploy"
./scripts/deploy.sh --config=$SCRIPT_DIR/config1.yaml &
./scripts/deploy.sh --config=$SCRIPT_DIR/config2.yaml &
./scripts/deploy.sh --config=$SCRIPT_DIR/config3.yaml &
wait

echo ">>> >>> >>> start init values"
./scripts/init_values.sh --values="100,100,100" --config=$SCRIPT_DIR/config1.yaml &
./scripts/init_values.sh --values="100,100,100" --config=$SCRIPT_DIR/config2.yaml &
./scripts/init_values.sh --values="100,100,100" --config=$SCRIPT_DIR/config3.yaml &
wait

echo ">>> >>> >>> start eICN"
screen -L -S session_eICN_1 -dm go run main.go --config=$SCRIPT_DIR/config1.yaml --log=logs/1.log --debug=$DEBUG
screen -L -S session_eICN_2 -dm go run main.go --config=$SCRIPT_DIR/config2.yaml --log=logs/2.log --debug=$DEBUG
screen -L -S session_eICN_3 -dm go run main.go --config=$SCRIPT_DIR/config3.yaml --log=logs/3.log --debug=$DEBUG
sleep 3s
...
echo ">>> >>> >>> start regist"
./scripts/regist_eICN.sh --config=$SCRIPT_DIR/config1.yaml --target-config=$SCRIPT_DIR/config2.yaml
./scripts/regist_eICN.sh --config=$SCRIPT_DIR/config1.yaml --target-config=$SCRIPT_DIR/config3.yaml
./scripts/regist_eICN.sh --config=$SCRIPT_DIR/config2.yaml --target-config=$SCRIPT_DIR/config1.yaml
./scripts/regist_eICN.sh --config=$SCRIPT_DIR/config3.yaml --target-config=$SCRIPT_DIR/config1.yaml
